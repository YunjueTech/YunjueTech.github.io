...
export async function generateMetadata(props: {
  ...
  const post = allBlogs.find((p) => p.locale === 'en' && p.slug === slug)
  if (!post) return
  const authorList = post.authors || ['default']
  const authorDetails = authorList.map((author) => {
    const a = allAuthors.find((p) => p.slug === author)
    return coreContent(a as Authors)
  })
...
export default async function Page(props: { params: Promise<{ slug: string[] }> }) {
  const params = await props.params
  const slug = decodeURI(params.slug.join('/'))
  const sortedEn = sortPosts(allBlogs.filter((p) => p.locale === 'en'))
  const sortedCoreContents = allCoreContent(sortedEn)
  const postIndex = sortedCoreContents.findIndex((p) => p.slug === slug)
  if (postIndex === -1) return notFound()
  const prev = sortedCoreContents[postIndex + 1]
  const next = sortedCoreContents[postIndex - 1]
  const post = sortedEn.find((p) => p.slug === slug) as Blog
  const authorList = post.authors || ['default']
  const authorDetails = authorList.map((author) => {
    const a = allAuthors.find((p) => p.slug === author)
    return coreContent(a as Authors)
